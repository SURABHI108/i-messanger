...
<body>
        <a href="http://localhost/project/room/setting/setting_menu.php" style="font-size: x-large">Back</a>
          <table width="100%">
                    <?php
                              include "config.php";
                              $sender_tab=find_table($_SESSION['msg']);
                              $q1="select * from `" . $sender_tab . "` where msg_format<>'text' order by other_user_id, curr_date";
                              $data=mysqli_query(database_connect(), $q1);
                              $temp=1;
                              while($row=mysqli_fetch_array($data))
                              {
                                        $filename="http://localhost/project/room/message/" . @$row[6];
